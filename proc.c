...
/*
a funcao cowfork foi baseada na fork que se encontra nesse arquivo 
*/
...
cowfork(void)
{
  ...
  // Copy process state from proc. aqui no if muda 
  ...
  *np->tf = *curproc->tf;
  ...
  //indica o compartilhamento
  curproc->compartilhada = 1;
  np->compartilhada = 1;
  for(i = 0; i < NOFILE; i++)
    if(curproc->ofile[i])
      np->ofile[i] = filedup(curproc->ofile[i]);
